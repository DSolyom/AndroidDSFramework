...
public class ExpandableView extends FrameLayout implements Runnable {
    ...
    private ExpanderStateListener mExpanderListener;
     ...
     * @param listener
     */
    public void setOnExpanderListener(ExpanderStateListener listener) {
    ...
    protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
        ...
        if (mHeightAt == -1) {
            ...
            if (mExpanderListener != null) {
                mExpanderListener.onStartup(mExpanderNeeded);
            }
    ...
    private void start() {
        ...
        if (mExpanderListener != null) {
            mExpanderListener.onExpandingStarted(mIsSmall);
        }
    ...
    public void run() {
        if (!mExpander.compute()) {
            refreshExpanderView();
            if (mExpanderListener != null) {
                mExpanderListener.onExpandingFinished(mIsSmall);
            }
            return;
        }
    ...
    /**
     * @interface ExpanderStateListener
     */
    public interface ExpanderStateListener {

        /**
         * called when expander is setup and decided if it is actually needed or the inside view is small enough
         *
         * @param expanderNeeded
         */
        void onStartup(boolean expanderNeeded);

        /**
         * called when expanding animation will started
         *
         * @param wasSmall
         */
        void onExpandingStarted(boolean wasSmall);

        /**
         * called when expanding animation is finished
         *
         * @param isSmall
         */
        void onExpandingFinished(boolean isSmall);
    }
}